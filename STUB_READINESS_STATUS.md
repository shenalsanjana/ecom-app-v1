...
| home-taxonomy-sections | Taxonomy follow-up B: home page renders department cards under Shop by category and a new Shop by design grid grouped by department, with links built from `designPath` | Done | subagent-driven-development | Done (`.worktrees/home-taxonomy-sections`, `feat/home-taxonomy-sections`, removed after merge) | Done | Applied (all 6 task groups) | Done (both deltas merged into `openspec/specs/`) | Done (`openspec/changes/archive/2026-09-01-home-taxonomy-sections/` + narrative at `openspec/archive/2026-09-01-home-taxonomy-sections.md`) | Completed - merged to `main` | **`npm run build` and `npm run test:e2e` still owe a green run** in CI or against the VPS - task 6.5 is archived deliberately unticked. Three findings parked by ruling, not fixed: the footer's six-link cap drains the first department in taxonomy order (spec-compliant, a product decision about column IA); the visible department Eyebrow is not `aria-hidden`, so a linear screen-reader read says the department name twice; and two tall tiles sit in a four-column row the moment a second department gains a design. Follow-ups C, D and E from taxonomy spec §9 remain open. | Merged `--no-ff` as `832ac37` (10 commits); branch deleted. Spec: `docs/superpowers/specs/2026-09-01-home-taxonomy-sections-design.md`. Plan: `docs/superpowers/plans/2026-09-01-home-taxonomy-sections.md`. Executed task-by-task with a fresh implementer and a task-scoped spec+quality review per task, then one whole-branch review on the most capable model, one fix wave and one scoped re-review. Validation on merged `main` (`832ac37`): `npm run test` OK 869/869 across 106 files; `npx tsc --noEmit` clean; `npm run check:contrast` all pairs and tints at AA; `npm run lint` 7 problems (4 errors, 3 warnings) every one in a file this change never touched. `npm run build` / `npm run test:e2e` BLOCKED-ENVIRONMENTAL - `DATABASE_URL` points at the unreachable docker-compose host `postgres`. The whole-branch review checked specifically whether anything here is uniquely gated on those two and found nothing: the page already awaited a Prisma-backed cached read before this change, and no e2e spec asserts on the home page. What the reviews caught, worth remembering: two of the three Important findings were defects in the OpenSpec artifacts rather than the code - the section-order delta asserted unconditionally that Shop by category renders, contradicting the threshold delta and false on production, and the task checklist was still entirely unticked at archive time. A third surfaced during sync: the single-read requirement claimed the page passes rows to every section that needs them, but `SiteFooter` renders on ~20 pages and must read the taxonomy itself. The one code defect was an accessibility gap against the delta spec's own normative rule: Men and Women share the sub-category "Oversized Graphic T-Shirts", so groups labelled by `subName` alone gave a screen-reader user two identical headings; the department name now sits inside the heading's accessible name. Also repaired pre-existing drift: `category-strip.tsx` (deleted) listed designs as "categories" and linked flat, and `site-footer.tsx` did the same on every page. |
