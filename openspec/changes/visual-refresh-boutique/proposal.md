...
- Refresh shadcn-derived primitives in `components/ui/` (button, card, input, badge, dialog, dropdown-menu, alert, separator, label, textarea) — variant tunings only, public APIs preserved. New `brand` variant on button + badge for olive treatments (see `design.md` "Brand vs hover token" decision; the brand olive lives on dedicated `--brand` / `--brand-foreground` tokens, distinct from shadcn's `--accent` subtle-hover semantic).
...
- **Intentional (not drive-by) accessibility fixes** bundled in: missing accessible name on stars in `product/reviews-section.tsx`, ungrouped quantity-stepper buttons in `cart/cart-item.tsx`, focus rings on the wishlist heart and free-shipping bar, 40 × 40 CSS px hit targets on header icons (currently 32 px). Surfaced explicitly in the audit because they would otherwise be invisible after the visual changes ship. (A fourth claimed bug — `h2 → h4` jump in `home/category-strip.tsx` — was disproved during implementation; documented as N/A in tasks.md §3.5.)
