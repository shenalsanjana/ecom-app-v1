...
- [ ] 2.1 Rewrite `app/_lib/order-validation.ts` to validate against both pools (Plan Task 4)
- [ ] 2.2 Rewire `app/checkout/actions.ts` to acquire both pools and snapshot pool ids onto `OrderItem` (Plan Task 5)
- [ ] 2.3 Simplify `applyItemChanges` in `app/_lib/admin-orders.ts` to pure item math (Plan Task 6)
