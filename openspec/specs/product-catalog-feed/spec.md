...
Defines the public CSV catalog feed that Meta Commerce Manager / Facebook Shop ingest:
its location, format, caching, the field mapping from each product **color variant** to
Meta's catalog schema (including the inverted on-sale price convention and availability),
and the shared `item_group_id` invariant (the design's `product.id`) that keeps a design's
colors grouped and aligned with the Pixel and JSON-LD.
