...
its location, format, caching, the field mapping from the Product model to Meta's
catalog schema (including the inverted on-sale price convention and availability), and
the shared product-id invariant that keeps the feed aligned with the Pixel and JSON-LD.
