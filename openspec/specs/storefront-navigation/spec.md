...
Defines how the storefront exposes the Department → Design taxonomy through
navigation chrome — the header panel, its degenerate fallback, and the mobile
menu — and the server/client boundary that navigation must respect. The
taxonomy model itself lives in `storefront-taxonomy`; this capability covers
only the surfaces that navigate it.
