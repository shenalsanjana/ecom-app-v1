...
Defines the shared raw-material inventory model that backs storefront and admin availability: plain T-shirt stock as one quantity per color+size shared across every product offering that color+size, and DTF print design stock as one quantity per design shared across every product built on it. Covers the admin Inventory section that manages both pools, the purchasability rule that combines them, and the atomic acquire/restore behavior at checkout, cancellation, edit, and payment failure.
