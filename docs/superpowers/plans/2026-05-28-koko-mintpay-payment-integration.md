...
**SG-4 — Mintpay payload field length limits (DRF `max_length`):** Symptom observed 2026-05-30 in live mode — `orderCreate` returned `400 {"message":"client side error","code":400,"errors":{"customer_id":["Ensure this field has no more than 10 characters."]}}`. Root cause was a code bug: `customer_id` was set to the customer email (>10 chars). Fixed in `app/_lib/payments/mintpay.ts` by deriving a stable opaque 10-char id via `shortCustomerId(email)` (sha256 hex slice). `customer_id` is safe to make opaque because only `order_id` is matched on the Mintpay return URL. **Remaining risk:** Mintpay validates every field with the same DRF length rules. Our `order_id` is the full order id (e.g. `ORD-1780120079104-EWYNOZ`, ~24 chars). If Mintpay's `order_id` `max_length` is < that, the next 400 will name `order_id` — and unlike `customer_id`, it CANNOT be hashed/truncated because the Mintpay return handler (`app/api/payments/mintpay/return/route.ts`) looks the order up by it. If that surfaces, the fix is to send `webNumber` (e.g. `WEB1001`) as Mintpay's `order_id` and map back to the internal id on return — a real change, not a one-liner. Also spot-check `customer_telephone` and `delivery_*` against Mintpay's field spec. **A passing unit test on `customer_id.length <= 10` proves the local constraint, not gateway acceptance — only a real sandbox/live order clears the full payload. Place one before declaring Mintpay done.**
