...
**SG-3 — Koko `merchantPluginDetail.notExists` 400 at orderCreate (production provisioning):** Symptom observed 2026-05-30 in live mode — `POST https://prodapi.paykoko.com/api/merchants/orderCreate` returns `{"status":"CLIENT_ERROR","statusCode":400,"validationFailures":[{"field":"onlineOrderCreate","code":"OnlineOrder.create.merchantPluginDetail.notExists"}]}`. This is a **Koko-account provisioning error, not a code bug** — the option appears at checkout and initiation reaches the gateway correctly. Koko keys every integration on the tuple **`merchantId` + `pluginName` + `pluginVersion`** (sent as `_mId` / `_pluginName` / `_pluginVersion` in `app/_lib/payments/koko.ts`). The 400 means no such "merchant plugin detail" record exists in Koko's production system for that tuple. Root-cause checklist:

- **`KOKO_PLUGIN_NAME` / `KOKO_PLUGIN_VERSION` must match what Koko provisioned.** When unset, `getKokoConfig()` (`app/_lib/payments/config.ts`) defaults to `customapi` / `1`. These are placeholders, **not guessable** — obtain the exact registered values from Koko (merchant portal integration page or onboarding contact) and set both in Vercel (Production + Preview). They were absent from Vercel when this error first occurred.
- **Credentials must match the environment being hit.** `KOKO_MODE=live` targets `prodapi.paykoko.com`, which requires the **production** `KOKO_MERCHANT_ID` / `KOKO_API_KEY` / keys. QA/sandbox credentials against the prod host fail with this same error.
- **The integration must be activated for production** by Koko's team, not just created.

Fast isolation ladder: (1) temporarily set `KOKO_MODE=test` + redeploy to hit `qaapi.paykoko.com` — if it works, the prod merchant/plugin is unprovisioned or prod creds are wrong; if it fails identically, the `pluginName`/`pluginVersion` tuple is wrong in both environments. (2) Confirm with Koko the exact `pluginName`/`pluginVersion` for your live merchant ID and that production is activated. Reminder: env-var changes only take effect on a **new** deployment.

**SG-3 UPDATE (root-caused against QA 2026-05-30):** Reference material is in `tmp/koko/` (API PDF v1.05, order-view PDF, `koko.php`, `phpqa.php`, credentials note). Two findings:

1. **`merchantPluginDetail.notExists` = environment mismatch, confirmed.** The note in `tmp/koko/` states the merchant `c8cca514…` / API key `83fA5n…` / RSA keys are **QA-only** credentials ("production keys provided later"). Vercel had `KOKO_MODE=live` → those QA creds hit `prodapi.paykoko.com` → plugin not found. Reproducing the exact payload against **`qaapi.paykoko.com`** changes the error to `signature.invalid` (plugin IS found). So the fix for *this* error is **`KOKO_MODE=test`** (we only have QA creds; live is unreachable until Koko issues prod keys). The `pluginName=customapi` default is correct; `phpqa.php` uses `pluginVersion=1.0.1` (vs our default `1`) — set `KOKO_PLUGIN_VERSION=1.0.1` to match the QA sample, though version did not affect the signature result below.

2. **`signature.invalid` is a Koko-side QA blocker — NOT a code bug.** `KOKO_MODE=test` is necessary but **not sufficient**. Verified: our RSA signing matches the v1.05 spec's `dataString` order byte-for-byte (`koko.ts:33-48`), and the algorithm (RSA-SHA256, PKCS#1 v1.5, base64) is deterministic and identical to the PHP samples. Reproduced **three** documented payloads against QA — ours, `koko.php` (single URL), and `phpqa.php` (distinct return/cancel/response URLs, which rules out any dataString reconstruction-order issue) — **all rejected with `signature.invalid`** using Koko's own documented QA private key. Conclusion: the public key Koko's QA server has registered for `c8cca514…` does not match the private key in these docs, so no signature we produce can validate. **Escalate to Koko** (do NOT change signing code): "Signing your own QA sample payloads (`koko.php`/`phpqa.php`) with the QA private key you provided yields `OnlineOrder.create.signature.invalid` from `qaapi.paykoko.com/api/merchants/orderCreate`. Please confirm the RSA public key registered for merchant `c8cca514…`, or provide the private key matching it." (The Mobile `765283630` / Password `Madhu@123` in the note are the Koko *shopper* gateway login used during the payment redirect, not API credentials.)

**SG-4 — Mintpay `customer_id` must be a 32-bit-int-safe NUMBER (root-caused against sandbox 2026-05-30):** Two-stage failure, both fixed in `app/_lib/payments/mintpay.ts`:

- **Stage 1 (400):** `customer_id` was originally the customer email (>10 chars) → `400 {"errors":{"customer_id":["Ensure this field has no more than 10 characters."]}}`.
- **Stage 2 (500):** First fix used a 10-char sha256 **hex** slice. That passes the `max_length=10` serializer check, but Mintpay then casts `customer_id` to an integer server-side — a hex string (letters) throws and returns a generic Django `Server Error (500)` HTML page (no body).

**Verified contract** (reproduced and bisected directly against the sandbox `dev.mintpay.lk/user-order/api/` with documented creds `sb0001` / `0af394cfb59ca9827e8fbf7b6473746d1c9cdad6`): `customer_id` must be **numeric, ≤10 chars, and ≤ 2147483647** (signed 32-bit). `9999999999` (>2^31) also returns 500 — proves an int-column overflow. Final fix derives a stable 9-digit number from the email: `parseInt(sha256(email).slice(0,8),16) % 1_000_000_000` → `0..999,999,999`, always in range; deterministic per buyer. `customer_id` is not a reconciliation key (only `order_id` is matched on the return URL), so a derived value is safe.

**Fields verified FINE by the same sandbox bisect (do NOT "fix"):** long non-numeric `order_id` (`ORD-…-H84YKF` → 200 — the earlier "order_id length" worry was wrong; an overflow on a serializer-validated field returns a structured 400, never a 500), cuid `product_id` (spec types it `String`), empty `delivery_postcode`, 2-dp `total_price`/`unit_price`, `ip=0.0.0.0`. The full real payload returns `{"message":"Success","data":<purchase_id>}`.
